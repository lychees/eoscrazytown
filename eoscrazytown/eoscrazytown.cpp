...
void crazytown::init(const checksum256 &hash) {
    require_auth( _self );
...
void crazytown::test(const account_name account, asset eos) {
    require_auth(_self);    
...
real_type crazytown::grief_ratio() const {
...
void crazytown::buy(const account_name account, asset eos) {
    /*
//    require_auth( _self );     
...
void crazytown::sell(const account_name account, asset dmt) {
...
void crazytown::send_referal_bonus(const account_name referal, asset eos) {
    if (!is_account(referal)) return;    
    eos.amount /= 200; // 0.5%
    if (eos.amount == 0) return;
}

void crazytown::bet(const account_name account, const account_name referal, asset eos, const checksum256& seed, const uint64_t bet_number) {
...
void crazytown::betreceipt(const rec_bet& bet) {
    require_auth(_self);
}
void crazytown::receipt(const rec_reveal& bet) {
    require_auth(_self);
}
void crazytown::buyreceipt(const rec_buy& bet) {
    require_auth(_self);
}
void crazytown::sellreceipt(const rec_sell& bet) {
    require_auth(_self);
}

void crazytown::onTransfer(account_name from, account_name to, asset eos, std::string memo) {        
    if (to != _self) {
        return;
    }
...
void crazytown::reveal(const checksum256 &seed, const checksum256 &hash) {
    require_auth(_self);
...
void crazytown::transfer(account_name from, account_name to, asset quantity, std::string memo) {        
    if (to == _self) {
        sell(from, quantity);
    } else {
...
uint64_t crazytown::get_bonus(uint64_t seed) const {
    seed %= 100;
    return seed;
}

uint64_t crazytown::merge_seed(const checksum256 &s1, const checksum256 &s2) const {
    uint64_t hash = 0, x;
    for (int i = 0; i < 32; ++i) {
 ...
 void crazytown::deal_with(eosio::multi_index<N(offer), offer>::const_iterator itr, const checksum256 &seed) {
...
checksum256 crazytown::parse_memo(const std::string &memo) const {
...
void crazytown::set_roll_result(const account_name account, uint64_t roll_number) {
